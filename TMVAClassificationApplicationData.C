...
void TMVAClassificationApplicationData( TString myMethodList = "" )
{
   ...
   Float_t mll, lead, sublead;
   reader->AddVariable( "mll", &mll );
   reader->AddVariable( "lead", &lead );
   reader->AddVariable( "sublead", &sublead );
  ...
  /*{ Float_t spec1,spec2;
   ...
   //
   TFile *input(0);
   TString fname = "RootFiles/data.root"; //For Real Data Samples
  ...
  /*{ else {
   ...
   std::cout << "--- Select signal sample" << std::endl;
    TTree* theTree = (TTree*)input->Get("data"); //For Data
    
   theTree->SetBranchAddress( "mll", &mll );
   theTree->SetBranchAddress( "lead", &lead );
   theTree->SetBranchAddress( "sublead", &sublead );
